  ...
  return (
    <Stack pt={8} px={{xs : 10 , sm : 20 , md : 10 , lg : 70}} bgcolor={"#F8FAFC"} >
      <Breadcrumbs separator=">">
        ...
        <Typography color="#000" variant="breadcrumbs"> المفضلة </Typography>
      </Breadcrumbs>
      {!loading ? <Box pt={24} pb={65}>
        {fav?.length > 0 && <Stack direction={"row"} alignItems={"center"} justifyContent={"space-between"} mb={16}>
          <Typography fontSize={20}> المفضلة : <span style={{color : "#0A5C99"}} >{fav?.length}</span> </Typography>
        ...
        {fav?.length == 0 && <Stack alignItems={"center"} spacing={24} pt={16}>
          ...
          <Stack spacing={4} alignItems={"center"}>
            <Typography variant="h2" color={"#02111D"} textAlign={"center"} > لا يوجد اي منتجات في المفضلة </Typography>
